...
class CarRacingRewardWrapper(gymnasium.Wrapper):
    """Reward shaping focused on progress, steering smoothness, and time."""
    ...
    def step(self, action):
        ...
        if action is not None:
            ...
            gas = float(action[1]) if len(action) > 1 else 0.0
            ...
            progress_bonus = 1.0 * new_tiles  # Increased to make tile hits very positive

            shaped_penalty = steering_smoothness_penalty
            reward += progress_bonus - shaped_penalty
        ...
        if new_tiles == 0 and self.no_progress_penalty:
...
class CarRacingGrayScaleWrapper(gymnasium.ObservationWrapper):
    """Convert RGB observations to grayscale while preserving native actions."""
...
def make_car_racing_env(
    ...
    if lap_complete_percent is not None:
        kwargs["lap_complete_percent"] = lap_complete_percent
    ...
    if grayscale_obs and not legacy_preprocessing:
